...
- **案S2 … 独立ページ `/events/:id/staffing`（#393 案P1 の型）。**
  ...
  「不足のみ」「自分の持ち場」フィルタ。格子と `EventSchedule`（一覧）には
  **読み取り専用のチップ表示だけ**足す。
...
**推しは案S2**（操作は独立ページ、格子・一覧はチップ表示のみ）。
「埋まっていない持ち場」を並べて潰していく作業動線が要件3の中心で、
格子はそれの確認画面という役割分担が素直。S1 の「格子から直接ダイアログを開く」は
S2 の後から足せる（同じダイアログを使い回す）。

### 8.2 タグの見え方（要件4）

- staff の格子（案V1 の列を含む）と `EventSchedule` 一覧の項目に、役割チップを出す:
...
- **自分に付いたタグ**: staffing ページの「自分の持ち場」フィルタと、
  格子のチップの強調（自分が入っている持ち場は塗る）。これが「割り当てられた
  スタッフにタグが付いて見える」の実体
...
タイムテーブルに混ざる語（チップ・不足表示）は `schedule.ts`、
staffing ページの語は `staffOps.ts`（#383 7.5 / #393 8.6 の線引きのとおり）。
`ja` / `en` の2つ。`duty` / `slot` / `assignee` の実装語を UI に出さない
（「役割」「持ち場」「担当」）。

### 8.5 相談したい点

1. 置き場: **案S2 で良いか**（S1 併用を最初からやるか、後回しか）
2. `EventSchedule`（イベント詳細の時刻順一覧）にもチップを出すか、格子だけにするか
3. **兼務の重なり警告**（同じ人が時間の重なる2つの持ち場に居る）を v1 に入れるか。
   時刻は `computeScheduleTimes` の結果から出せるので画面側の導出だけで書けるが、
   裏方の時刻は明示のみ（#383 3.3）なので「時刻の無い持ち場」は判定から漏れる。
   v1 では見送り、別 issue が候補（10. に仮置き）
...
- **兼務の時間重なり検出は v1 でやらない**（8.5 の相談次第。やるなら画面側の導出のみで別 issue）
...
| `packages/shared/src/i18n/messages/schedule.ts` / `staffOps.ts` | 文言（8.4） |
| `apps/web/src/api/dutyHooks.ts` | **新規**（staff のときだけ有効化。8.3） |
| `apps/web/src/lib/dutyBoard.ts` | **新規**（充足の導出の純関数） |
| `apps/web/src/pages/EventStaffingPage.tsx` ほか（8. で決めた案） | **新規** |
| `apps/web/src/components/TimetableGrid.tsx` / `EventSchedule.tsx` | チップ表示（読み取りのみ） |
| `apps/web/src/App.tsx` / `EventDetailPage.tsx` | 子ルート1行・導線ボタン1つ |
| `apps/server/test/staff-duty.test.ts` | **新規**（9.1〜9.3・9.5・9.6） |
...
7. `dutyHooks.ts` → staffing ページ（8. の相談で決めた案）→ 格子・一覧のチップ表示
