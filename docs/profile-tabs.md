...
| タブ | key | 母集団 | 出す相手 |
|---|---|---|---|
| 参加予定イベント（**既定**） | `upcoming` | 公開済み・これから（`scheduling` または `endsAt >= now`）・`myRole !== "staff"` | 全員 |
| 参加した過去イベント | `past` | 公開済み・過去・`myRole !== "staff"` | 全員 |
...
- 主催イベントは予定でも過去でも `hosted` に置く。「参加予定」と二重掲載しない
  （1イベントはちょうど1タブ。4分類が保っていた排他をそのまま守る）
