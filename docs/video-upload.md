...
| イベントあたり本数 | 写真と共有の `EVENT_PHOTO_LIMIT = 200` 枠。動画専用の追加上限は設けない | 最悪 200 本 × 40MB = 8GB/イベント。R2 保存 $0.015/GB·月 → **$0.12/月/イベント**、egress 無料。上限を増やす複雑さに見合う節約がない |
...
- **途中離脱・失敗**: サーバに中間状態を作らない（§7.1 の原子性）ので、
  ...
  最後にまとめ画面へ理由つきで出す。200枠切れ（`photo_limit`）が出たら以降も
