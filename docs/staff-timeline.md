...
`useEventSchedule` を呼んでいる画面は 2・3・4 と `SchedulePanel`（日程調整。項目を描画しない）
の4つだけで、**すべて 1 の API 1本に集まっている**。フロントは 1 を塞げば塞がる。
...
4. **`tracks` を送らないクライアントからの保存では `visibility` を見ない**
   （既存値をそのまま書き戻す）。
   **実装中に見つけた穴。** 6.2 は `visibility` の既定を `'public'` と決めているが、
   `tracks` 未指定は「トラックを知らないクライアント」の経路で、トラックを知らない
   クライアントは**裏方も知らない**。既定値の `'public'` を素直に書くと、
   入力済みの裏方が黙って参加者に出る。`placement` を既存値のままにしているのと
   まったく同じ理由なので、同じ規則を `visibility` にも適用した。
