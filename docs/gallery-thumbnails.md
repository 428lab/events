...
with long edge at most 480px, aspect preserved and no upscale. Event grid only uses
...
- Browser uses existing WebP/JPEG encoder, 480px, quality .8 then .5; max 128KiB.
...
- Server checks thumbnail bytes, WebP/JPEG signature and encoded dimensions
  (1..480). This is bounded header validation, not a full image decoder, like
  existing video validation. A submitted invalid thumbnail rejects before writes.
