...
- ステータス: **実装済み**（PR #461。設計からの差分は 9.、実装時に確認した点は 10.）
...
|------|------|----------|------|----------------|
| 40 | チャンネル作成 | `routes/eventChat.ts`（公式鍵）/ `nostrChat.ts` `buildChannelCreateTemplate`（主催者 NIP-07） | 公式鍵 or 主催者鍵 | **参加者のブラウザ**（`components/chat/useChatChannel.ts` の `pool.publish(created)`） |
...
- kind:40 だけが「署名者（公式鍵）と発行接続（参加者）が別人」で、NIP-70 の
  「AUTH 済み pubkey ＝ イベントの pubkey」制約に反する。`components/chat/useChatChannel.ts` の
  チャンネル確定処理にこのリスクの注記が既にある（リレー側の特例設定で運用する前提だった）。
...
- 主催者が NIP-07 で参加している場合はブラウザが `buildChannelCreateTemplate` で
  自ら署名する分岐がある（`components/chat/useChatChannel.ts` の `isOrganizerNip07`）。
...
- 発言者は自分の鍵で NIP-42 AUTH してから発言する（`ChatRelayPool` 実装済み）ので、
  ...
  タグ 1 つだけ**（ただしこの前提には穴があった。9. の #464 を参照）。
...
- `POST /:id/chat-channel`（主催者経路の登録。`registerChatChannelInput` の body）と
  `buildChannelCreateTemplate`（`["-"]` 付きに変更）、`components/chat/useChatChannel.ts` の
  主催者 NIP-07 分岐。
...
| 案 | 判断 |
|----|-----------|
| リレー側に「公式鍵のイベントは誰の接続からでも受理」の特例を入れる（現行コメント `components/chat/useChatChannel.ts` の運用案） | 封じ込めの契約がアプリの外（リレー設定）に漏れる。カスタムリレー運用者に同じ特例を要求することになり、素の NIP-42/70 対応だけで動く、という 3.5 の要件文が書けなくなる |
...
2. **PR-2: web の追随と文言・docs 同期**（web + shared + docs）
   - `buildChannelMessageTemplate` / `buildChannelCreateTemplate` に `["-"]`、
     チャンネル確定処理（#335 以降は `components/chat/useChatChannel.ts`）の公式鍵側フローの置き換え（`createOfficialChannelEvent` +
...
| `apps/web/src/components/chat/useChatChannel.ts`（#335 で `EventChat.tsx` から分割） | 公式鍵側の「署名取得→publish→登録」を `POST /chat-channel/create` 1 発に置き換え（`organizerNip07` 分岐は維持） |
...
- **AUTH の署名待ちに期限を入れ、AUTH の入り口を 1 つにした（#464。後追いの
  修正 PR）**。3.1 の「クライアント側の変更はタグ 1 つだけ」は、**AUTH が毎回
  走るようになる**という副作用を見落としていた。全発言が protected になった
  ことで、リレーは必ず auth-required を返し、NIP-07 利用者は 1 通ごとに拡張の
  署名ダイアログを見る。nostr-tools 2.24.1 の `relay.auth()` は署名器が投げても
  resolve も reject もせず、その promise を接続が生きているあいだ使い回すため、
  ダイアログを拒否・放置されると `ChatRelayPool.publish` が永久に待ち、送信が
  成功とも失敗とも表示されなかった（一時鍵の経路は自動署名なので露出しない）。
  制約と決着:
  - **AUTH の入り口は 2 つある**。こちらから `auth()` を呼ぶ経路と、リレーが
    送る `["AUTH", challenge]` を受けて nostr-tools が自分で `onauth` を呼ぶ
    経路（10.2 のとおり strfry は接続直後には送らないが、送るリレーはある）。
    後者が先に走ると `authPromise` が先に出来上がり、こちらの期限も例外処理も
    素通りされる。**署名は `signAuth` 1 か所を通し**、失敗はリレーごとの
    `AuthState` に集めて、待っている側へ通知する。
  - **期限は署名の段にだけ掛ける**（`AUTH_SIGN_TIMEOUT_MS` = 15 秒。人が
    ダイアログを操作する時間）。リレーとの往復には nostr-tools 自身の
    `publishTimeout` (4.4 秒) が既に効いており、`auth()` はそのタイマーを署名が
    返ってから張る。両方を 1 本のタイマーで囲うと、12 秒で承認された署名を
    15 秒で切って、認証できたばかりの接続を捨てることになる。段ごとに順に待つ。
  - **AUTH に失敗した接続は捨てて張り直す**。宙吊りの `authPromise` を抱えた
    ままでは、次の送信で署名を求めることすらできない（`connect()` が捨てるので
    繋ぎ直せば署名からやり直せる）。ただし**張り直すのは次の送信の直前**。
    失敗した瞬間に張り直すと、接続直後のチャレンジで「何も送っていないのに
    署名ダイアログが出る」ことになる。
  - **読み取り側（購読の張り直し）は失敗しても接続を捨てない**。読み取りは
    利用者の操作と無関係に何度でも起きるので、捨てて繋ぎ直すと署名を断った人に
    ダイアログを出し続けることになる。購読は次の送信か再接続で復旧する
    （`connectOne` が購読を張り直すため、送信 1 回で読み書きとも直る）。
    署名を一度も承認しない読み取り専用の利用者は読めないままだが、投影用画面
    (#215) は一時鍵で自動署名するのでこの状態にはならない。
  - publish の「全滅なら 1 回だけリトライ」(#225) は**そのまま無条件で残す**。
    AUTH を断られたリレーは `authenticate` が待たずに失敗するので、リトライは
    「もう 1 台が単に切れていただけ」の救済としてだけ働き、ダイアログも増えない。
