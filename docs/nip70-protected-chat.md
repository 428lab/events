...
- **AUTH 待ちに期限を入れた（#464。後追いの修正 PR）**。3.1 の「クライアント側の
  変更はタグ 1 つだけ」は、**AUTH が毎回走るようになる**という副作用を見落として
  いた。全発言が protected になったことで、リレーは必ず auth-required を返し、
  NIP-07 利用者は 1 通ごとに拡張の署名ダイアログを見る。nostr-tools 2.24.1 の
  `relay.auth()` は署名器が投げても resolve も reject もせず、その promise を
  接続が生きているあいだ使い回すため、ダイアログを拒否・放置されると
  `ChatRelayPool.publish` が永久に待ち、送信が成功とも失敗とも表示されなかった
  （一時鍵の経路は自動署名なので露出しない）。
  制約と決着:
  - **署名器の失敗は自前で拾う**（拒否は即時失敗）。待ち切りは「ダイアログを
    放置された」場合だけに効かせる。
  - **期限は人がダイアログを操作する時間**（`AUTH_TIMEOUT_MS` = 15 秒）。接続や
    EVENT の待ちとは別物なので、nostr-tools 既定の 4.4 秒には合わせない。
    期限は 1 か所に置き、publish と購読の張り直しが同じ経路（`authenticate`）を通る。
  - **AUTH に失敗した接続は捨てて張り直す**。宙吊りの `authPromise` を抱えたまま
    では、次の送信で署名を求めることすらできない（`connect()` が捨てるため、
    繋ぎ直せば署名からやり直せる）。
  - publish の「全滅なら 1 回だけリトライ」(#225) は AUTH 失敗では回さない。
    繋ぎ直して即やり直しても結果は同じで、署名ダイアログを二度出すだけになる。
