...
- メール送信・OG取得の**予算**（`takeEmailSlot` / `takeOgFetchSlot`）は今も
  アイソレート共有のモジュール変数のまま。並行リクエストがあると**両方向にズレる**。
  - **少なく見える方向**: 並行リクエストが枠を使うと、走っている側の残りが減る。
    そのリクエストで送れる数は減るが、一斉連絡は送れなかったぶんを送信待ちへ戻すので宛先は落ちない。
  - **多く見える方向**: `bindEnv` は毎リクエスト予算を 20 に戻すので、
    並行リクエストが入ると走っている側の予算まで回復し、1リクエストで 20 を超えて送りうる。
  どちらもサブリクエスト上限（有料で 1000/リクエスト）に対して桁が違い、
  暴走防止という目的は満たす。実行文脈と違って**取りこぼしは起きない**ので、
  リクエストごとに持ち直すところまではやっていない。
...
- **失敗方向は「孤児」に倒す。** どちらを先にしても失敗はしうるので、
  どう壊れるかを選ぶ話になる。R2 が先に成功して D1 が失敗すると
  **参照はあるのに実体が無い**（一覧に出るのに開けない・運営の対処の証跡 #278 も
  消えている・行から復元できない＝回復不能）。D1 が先なら残るのは
  **誰からも参照されない実体**で、配信はされず prefix を舐める掃除で後から拾える。
- **列挙は `bucket.list` ではなく D1 から。** 何がそのイベントの持ち物かを知っているのは D1 で、
  景品画像 (#434) のキーは乱数入りで D1 にしか無い。
  掃除用の SELECT は**表示用の絞り込みを通さない**（運営が非表示にした写真を落とすと、
  その実体だけが残る）。
- **`deferBackground` に逃がさない。** 収集はどのみちインラインになり、残る R2 側は
  1回の multi-delete（R2 は1回 1000 キー、イベント写真は `EVENT_PHOTO_LIMIT` 本まで）。
  サブリクエスト予算 50 に対して余裕があり、インラインなら失敗が
  テストとレスポンスから見える。
...
- アイコンは**ログインのたびに**、そのとき使った連携先（Discord / Google / GitHub / X などのOAuth経路）から取り直して R2 `avatars/{user_id}` に保管し、`avatar_url` を自分のドメインのURLへ差し替える (#312)。連携先のURLをそのまま持つと、向こうでアイコンを変えられた時点で 404 になるため。
...
- Nostr の `POST /api/auth/nostr/profile` はログイン時の追従更新と分け、**補完前にアイコンが未設定のときだけ**自前保管を試みる (#389)。設定済みなら取得自体を呼ばず、DBのURL・画像メタデータ・R2実体を保護する。表示名の未設定時補完は続ける。補完後のURLで判定すると初回取り込みまで止めるため、事前の値で判定する。既に上書きされたデータの自動復元や、処理中の並行編集との競合制御は本修正の対象外。
...
- **イベント詳細**: 内容表示、参加登録・解除ボタン、参加者一覧、成果物（自分の Entry の URL 編集）。
  - 参加者一覧は保存済み `event_member.slot_id` で募集枠別に表示する (#530)。従来の平坦な一覧は枠も参加状態も表示せず、抽選申込者の選択が見えなかった。枠APIの順序・名称、枠内の既存登録順を保ち、抽選申込中・確定・キャンセル待ち・落選を同じ枠に置いて各行に既存の日英文言で状態を併記する。枠人数を確定人数として新設せず、定員集計は既存APIのままとする。
  - 枠に対応しない人（スタッフ、枠なしの旧登録、参照先が取得できない登録）は末尾の「その他」に残し、役割も表示する。枠未取得・取得失敗・枠なしイベントでは従来の全員一覧を維持する。登録の復元・選択の推測はせず、アンケート回答や日程候補から枠を導出しない（複数枠の日程自動登録は本人選択待ち）。APIの取消・退会除外、出席表示設定、スタッフ操作と権限は変えない。スキーマ・移行・通知・状態遷移の変更は不要。
  - 受入: 複数枠それぞれの確定/抽選申込者が正しい枠に一度だけ現れ、状態を区別できること。枠なし・未知枠・スタッフも落とさず、枠取得失敗時も一覧を失わないこと。既存の出席操作テストに加え、この表示契約を狭い回帰テストと隔離コピーの枠判定除去変異で検証し、実アプリの詳細URLを日英・モバイル幅で確認する（業務APIフィクスチャは実認証受入と区別）。
