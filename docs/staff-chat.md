...
イベント作成者は作成時に staff / confirmed の行ができる（`routes/eventCrud.ts` の
...
ただし「捨ててよい候補」からは**自分の鍵を除く** (#335)。許可リストは〜5秒遅れて
届くので、発言した直後の本人はリスト上「部外者」に見える。満杯のバッファに本人が
発言すると、捨ててよい候補が**いま送った自分の発言だけ**になり、それが真っ先に
消える（リレーは再送しないので二度と出ない）。混雑した場に入った直後ほど起きる。
