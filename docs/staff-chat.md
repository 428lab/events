...
- ステータス: **設計のみ（未実装）**。着手前にユーザーと方式の擦り合わせを行う
...
| 画面 | `EventChat.tsx` (**953行**, #360) | 800行制約を既に超過。**ここには足さない** |

重要な性質: **本文はブラウザ⇔リレー直通**で、サーバーはメッセージを見ない。
NIP-70 は strfry がコアで拒否するため不採用で、封じ込めは
「書き込みは自リレー2台限定（`wss://r.kojira.io` / `wss://x.kojira.io`）＋NIP-42 AUTH」。
...
| 退会（完全削除） | `users.ts` `deleteAccount`（purge #250） | event_member は FK CASCADE で消える。**ルートを通らない**のでここにもフックが要る |
...
|---|---|
| kind 9807 の暗号文が存在すること・投稿時刻・おおよそのサイズ | NIP-44 v2 はパディング付きで正確な長さは丸まる。「どこかの運営が何時に何通話したか」までは露出する。**許容する** |
...
## 6. スキーマとマイグレーション（0074_staff_chat.sql）
...
呼び出し箇所は 2.6 の3経路（漏れると「抜けた人が新しい発言を読める」が残る）:
...
| 退会 purge | `users.ts` `deleteAccount` 内。削除前に「confirmed staff だったイベント」を列挙して各部屋を回す（signer 行自体は CASCADE で消える） |
...
- リレー側の変更（relay29・NIP-70 等）。素の strfry のまま
...
1. **PR1（server）**: 0074 マイグレーション＋リポジトリ＋ routes/staffChat.ts ＋
