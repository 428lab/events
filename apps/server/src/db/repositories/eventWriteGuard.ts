...
export type EventWriter = {eventId:string;actorId:string;permission:"view"|"manager"|"staff"|"judge"|"member"|"chat-member"|"chat-staff"|"registered"};
...
export function eventWriteGuard({eventId,actorId,permission}: EventWriter): Statement {
  const role = permission === "view" ? "1" : permission === "manager"
    ...
    : `EXISTS(SELECT 1 FROM event_member m WHERE m.event_id=e.id AND m.user_id=u.id AND m.status${permission === "registered" ? "<>'canceled'" : "='confirmed'"} AND m.role IN (${permission === "staff" || permission === "chat-staff" ? "'staff'" : permission === "judge" ? "'judge','staff'" : "'staff','participant','observer','judge'"}))`;
  return {sql:`SELECT CASE WHEN EXISTS(SELECT 1 FROM event e JOIN user u ON u.id=?2 AND u.deleted_at IS NULL
    WHERE e.id=?3 AND ${eventViewSql("e","u.id","?1")} AND (${role} ${permission === "member" || permission === "registered" || permission === "judge" || permission === "chat-member" ? `OR ${eventManagerSql("e","u","?1")}` : ""})
