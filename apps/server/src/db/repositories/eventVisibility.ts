...
export function visibilityChangeStatements(eventId:string,token:string,actorId:string,from:Event["visibility"],to:Event["visibility"]) {
  ...
  return [
    ...(to === "private" && from !== "private" ? [
      {sql:`INSERT INTO event_access_invite(id,event_id,user_id,invited_by,status,source,created_at,responded_at)
        SELECT lower(hex(randomblob(16))),m.event_id,m.user_id,?,'accepted','existing_member',?,?
        FROM event_member m WHERE m.event_id=? AND m.status<>'canceled' AND ${accessOperationGuard}
        ON CONFLICT(event_id,user_id) DO NOTHING`, args:[actorId,now,now,eventId,...args]},
