...
const r2Key = (liveSetId: string, imageId: string) =>
  `live-set-images/${liveSetId}/${imageId}`;

/** 公開: 配信シーン画像の取得（imageId は一意なので長期キャッシュ） */
export async function getLiveSetImage(c: Context<AppEnv>) {
...
/** owner: 配信シーン画像のアップロード（生バイナリ）。URL を返す */
export async function putLiveSetImage(c: Context<AppEnv>) {
  const id = c.req.param("id")!;
  const liveSet = await liveSetsRepo.findById(id);
  ...
  return c.json({ url: `/api/live-sets/${id}/images/${imageId}` });
}
