...
/** Client crops/encodes to WebP; never accept a different stored format. */
export async function putMyAvatar(c: Context<AppEnv>) {
  if ((c.req.header("content-type") ?? "").split(";")[0].trim().toLowerCase() !== "image/webp")
  ...
  const size = cardImageDimensions(new Uint8Array(bytes), "image/webp");
  ...
  const key = `${avatarUploadPrefix(userId)}${crypto.randomUUID()}.webp`;
  ...
  try {
    ...
    if (!await userAvatarsRepo.setUploadedAvatar(userId, previous.uploadedKey, previous.updatedAt, key, avatarUrl, at, hash)) {
