 ...
 *
 * **配下のサブアプリは自前の requireAuth を持たない (#472)。** Hono の
 * ミドルウェアはパターン一致で積まれるので、`/events` に並べたサブアプリが
 * 各自 `use("*", requireAuth)` を持つと、**どのハンドラを叩いても並べた全部が
 * 順に走る**（#472 の時点で最大23回＝1リクエストで D1 を46回）。
 * `test/auth-boundary.test.ts` が、登録済みルート1本ごとに requireAuth が
 * ちょうど1回であることと、未認証で通る経路が表のとおりであることを見張る。
 *
 * ## 中身がどこにあるか
 *
