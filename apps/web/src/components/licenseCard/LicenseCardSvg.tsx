 ...
 *
 * **このファイルはカード1枚の絵** (#466)。ヘッダー・アバター・表示名・
 * プロフィールパネル・コミュニティ帯は、互いの座標を見ながら1枚に収まるよう
 * 決めた1つのレイアウトで、絵でないもの（配色カタログ・座標定数・文字幅の見積り・
 * データ変換・QRの符号化）はすべて隣のモジュールへ出してある。
 *
 * ただし**コミュニティ帯 (#181) は切り出せなくはない**。帯の幅を決めている
 * QRの位置は `cardLayout.ts` の定数になったので、別ファイルにして読ませる形にできる。
 * 割っていないのは技術的に無理だからではなく、423行で困っていないから。
 * 切れば「帯が何を避けて幅を決めているのか」が2ファイルに分かれるぶん読みにくくなる、
 * というだけの理由なので、帯が別の判断（3行折り返し・アイコンの出し分け等）を
 * 抱えて育ったら、そのときは切るほうが勝つ。 */
