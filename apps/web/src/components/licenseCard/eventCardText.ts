...
export function fitCardText(text: string, width: number, height: number, preferred: number) {
  const paragraphs = text.split("\n");
  let best = { lines: paragraphs, size: 0 };
  const longest = Math.max(1, ...paragraphs.map(textUnits));
  for (let count = 1; count <= 4; count++) {
    ...
    for (const paragraph of paragraphs) {
      ...
      for (const char of paragraph) {
        if (line && textUnits(line + char) > limit) {
