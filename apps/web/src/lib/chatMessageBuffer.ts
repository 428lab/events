 ...
 *
 * **自分の鍵は許可リストと同じに扱う**（`bufferAllowPredicate`）。
 * ポーリングが追いつく前に発言した本人は、許可リストのうえでは「部外者」に
 * 見える。満杯のバッファへ本人が発言すると、捨ててよい候補が**自分の発言だけ**
 * になり、いま送ったばかりの発言が真っ先に消える（リレーは再送しないので
 * 二度と出ない）。混雑したイベントに入った直後ほど起きる。
 */
...
/**
 * バッファが「捨ててよくない」とみなす pubkey の判定。
 *
 * 許可リストに載っている人 ＋ **自分**。自分を足すのは、許可リストが
 * 〜5秒遅れて届くため。発言した直後の本人は許可リスト上まだ部外者であり、
 * 満杯のバッファでは「唯一の捨ててよい候補＝自分のいまの発言」になってしまう。
 *
 * 両方の画面で同じ規則なので、ここ1か所に置いて呼び出し側から渡す
 * （表示するかどうかは別の判定＝`selectVisibleChatMessages`。
 * こちらは許可リストだけを見る＝自分の発言も、載るまでは描かない）。
 */
export function bufferAllowPredicate(
  allowed: Set<string>,
  myPubkey: string | null | undefined,
): (pubkey: string) => boolean {
  return (pubkey) => allowed.has(pubkey) || pubkey === myPubkey;
}
