...
describe("encodeGalleryThumbnail", () => {
  it.each(["image/webp", "image/jpeg"])("keeps actual %s MIME and uses 480px", async (type) => {
    ...
    expect(encode).toHaveBeenCalledWith(source, 480, 0.8);
  });
  ...
  it("retries oversized output then omits if still too large", async () => {
    ...
    expect(await encodeGalleryThumbnail(source)).toBeNull();
    expect(encode.mock.calls.map(c => c[2])).toEqual([0.8, 0.5]);
  });
