...
describe("encodeGalleryThumbnail", () => {
  it.each(["image/webp", "image/jpeg"])("keeps actual %s MIME and requests a center-cropped 320px square", async (type) => {
    ...
    expect(encode).toHaveBeenCalledWith(source, 320, 0.8, true);
  });
  ...
  it("retries oversized output then omits if still too large", async () => {
    ...
    expect(await encodeGalleryThumbnail(source)).toBeNull();
    expect(encode.mock.calls).toEqual([[source, 320, 0.8, true], [source, 320, 0.5, true]]);
  });
