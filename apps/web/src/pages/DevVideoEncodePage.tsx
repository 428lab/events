...
/**
 * 動画エンコードの実機計測ページ (#408 の1段目)。
 *
 * 【一時ページ】ナビには載せず URL 直打ち（/dev/video-encode）でだけ開く。
 * 実機ごとのエンコード速度・選ばれる経路を体感/報告するためのもので、
 * 本実装の投稿 UI が入ったら削除する。そのため文言は ja 直書きで
 * i18n の縛りを緩めている（本実装の画面ではこの手抜きをしないこと）。
 *
 * パイプライン本体（lib/video/）は本実装でそのまま使う品質のコード。
 * 仮設なのはこの画面だけ。
 */
