...
export function useMyEventInvites() {
  ...
  return useInfiniteQuery({ queryKey: ["eventInvites", user?.id], enabled: Boolean(user), retry: false,
...
export function useMyEventAccess() {
  const { data: user } = useMe();
  return useInfiniteQuery({ queryKey: ["eventAccess", user?.id], enabled: Boolean(user), retry: false,
    initialPageParam: undefined as string | undefined, refetchInterval: 15_000,
...
export function useEventAccessInvites(eventId: string) {
  const { data: user } = useMe();
  return useQuery({ queryKey: ["event", eventId, "accessInvites", user?.id], enabled: Boolean(user), retry: false,
