...
- [x] 新規写真・動画ポスターのサムネイル: ブラウザで中央320×320pxの別画像を作り、イベント一覧で配信。拡大時は本体（[設計](docs/gallery-thumbnails.md)）。既存メディアの再生成は[staging準備・検証手順](docs/gallery-thumbnail-backfill.md)を参照（書き込みは独立レビュー後）。
